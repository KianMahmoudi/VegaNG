...
import com.kian.mahmoudi.vegang.dto.UrlContentRequest
import com.kian.mahmoudi.vegang.util.Constants.LOOPBACK
...
object HttpUtil {
    ...
    fun getUrlContent(request: UrlContentRequest): String? {
        ...
        if (request.httpPort != 0 && !request.proxyUsername.isNullOrBlank() && !request.proxyPassword.isNullOrBlank()) {
            requestBuilder.header(
                "Proxy-Authorization",
                Credentials.basic(request.proxyUsername, request.proxyPassword)
            )
        }
        try {
            client.newCall(requestBuilder.build()).execute().use { response ->
                if (!response.isSuccessful) {
                    Log.w(Constants.TAG, "Failed to get URL content, code=${response.code}")
                    return null
                }
                return response.body?.string()
            }
        } catch (e: Exception) {
            Log.e(Constants.TAG, "Failed to get URL content", e)
        }
        return null
    }
    ...
    fun downloadToFile(
        ...
        return try {
            client.newCall(requestBuilder.build()).execute().use { response ->
                if (!response.isSuccessful) {
                    Log.w(Constants.TAG, "Failed to download file, code=${response.code}, url=$url")
                    return false
                }
            ...
            Log.e(Constants.TAG, "Failed to download file: $url", e)
            false
        }
    }
}
